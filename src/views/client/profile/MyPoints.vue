...
const initData = async () => {
  try {
    // 1. 抓總積分
    const resTotal = await publicApi.get('http://localhost:8888/unicare_api/member/get_total_points.php')
    currentPoints.value = resTotal.data.total_points || 0

    // 2. 抓歷史紀錄
    const resHistory = await publicApi.get('http://localhost:8888/unicare_api/member/get_my_points.php')
...
const handleCheckIn = async () => {
  ...
  try {
    const res = await publicApi.get('http://localhost:8888/unicare_api/member/check_in.php')
